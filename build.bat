SET APP_DIR=build
SET GOARCH=386
FOR /F "tokens=*" %%g IN ('git --git-dir=%DELPHIPATH%\src\github.com\fpawel\aToolGui\.git rev-list -1 HEAD') do (SET GIT_COMMIT_GUI=%%g)
...
-ldflags "-X main.GitCommit=%GIT_COMMIT% -X main.GitCommitGui=%GIT_COMMIT_GUI% -X main.BuildUUID=%BUILD_UUID% -X main.BuildTime=%THE_TIME% -X main.BuildDate=%THE_DATE%" ^
